...
class IPCbuffer : public ObjectWrap {
 public:
  ...
  static inline char* Data(IPCbuffer *b) {
    return IPCbuffer::Data(b->handle_);
  }
  ...
  static inline size_t Length(IPCbuffer *b) {
    return IPCbuffer::Length(b);
  }
  ...
  static IPCbuffer* New(size_t length); // public constructor
  static IPCbuffer* New(char *data, size_t len); // public constructor
  static IPCbuffer* New(char *data, size_t length,
  ...
  IPCbuffer(v8::Handle<v8::Object> wrapper, size_t length, char* path, uint32_t id);
